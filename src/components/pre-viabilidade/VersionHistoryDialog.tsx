              ...
              {history.map((entry, idx) => (
                <div key={entry.id} className="relative border rounded-lg p-4">
                  ...
                  {idx < history.length - 1 && (
                    <div className="absolute left-7 top-full w-px h-4 bg-border" />
                  )}
                    ...
                    {entry.changed_fields.map((field) => (
                      <div key={field} className="flex items-start gap-2 text-xs">
                        ...
                        <span className="text-muted-foreground">
                          valor anterior: <span className="text-foreground font-medium">{formatValue(entry.snapshot[field])}</span>
                        </span>
                      </div>
                    ))}
                  </div>
